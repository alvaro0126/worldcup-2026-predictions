...
def _rec_row(r: Rec) -> dict:
    return {
        ...
        "Backup score": _score(r.alt_home, r.alt_away),
...
def _team_table(bm: BayesModel, teams: dict[str, Team], agg: SimAgg) -> pd.DataFrame:
    ...
    for name in sorted(teams, key=lambda t: bm.effective_elo(t), reverse=True):
        ...
        rows.append({
            ...
            "El Pais nivel": "" if t.nivel is None else t.nivel,
...
def _readme_lines(params: Params, val: Validation, agg: SimAgg) -> list[list[str]]:
    ...
    return [
        ...
        ["• 'REC SCORE' is the single most-likely scoreline. 'Backup score' is the most-likely score given the most-likely result (for pools that score the result separately)."],
        [""],
        ["MODEL (sound, reproducible)"],
        ["• Team strength = World Football Elo (eloratings.net), blended for lower teams with FIFA ranking + El Pais 'nivel'."],
        [f"• Match engine = Dixon-Coles Poisson, calibrated to El Pais anchors (alpha={params.alpha:.3f}, b={params.b:.3f}, rho={params.rho:.3f})."],
        ...
        [f"• Matches scored: {val.n}   Outcome hit-rate: {val.outcome_hit_rate:.0%}   Mean RPS: {val.mean_rps:.3f}  (El Pais reference ~0.17-0.18)"],
        [""],
        ...
        ["• Squad-value data (Transfermarkt) is proxied via Elo; El Pais weights it directly, which lifts star-squad teams (France/Brazil) slightly vs this model."],
        ["", ],
        ["TO UPDATE DAILY"],
